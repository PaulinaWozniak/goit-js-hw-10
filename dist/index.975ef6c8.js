...
searchBox.addEventListener("input", (0, _lodashDebounceDefault.default)((e)=>{
    e.preventDefault();
    if (searchBox.value) {
...
function showCountries(countries) {
    ...
    if (countries.length >= 2) secondMarkup(countries);
    if (countries.length === 1) firstMarkup(countries);
}
function secondMarkup(countries) {
    const secondMarkup1 = countries.map(({ name , capital , population , flags , languages  })=>{
        return `<li><img src="${flags.svg}" alt="Flag of ${name.official}" style="height: 30px; width: 50px"> ${name.official}</li>`;
    }).join("");
    countryList.innerHTML = secondMarkup1;
}
function firstMarkup(countries) {
    countries.map(({ name , capital , population , flags , languages  })=>{
        const markup = `<h1><img src="${flags.svg}" alt="Flag of ${name.official}" style="height: 60px; width: 60px; padding-top: 20px"> ${name.official}</h1>
